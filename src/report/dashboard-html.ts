...
const SCRIPT = [
  ...
  "    return '<tr><td>'+esc(r.metric)+'</td><td>'+fmt(r.base,1)+'</td><td>'+fmt(r.head,1)+'</td><td class='+cls+'>'+arrow+fmt(d,1)+'</td><td class='+cls+'>'+(ch>=0?'+':'')+ch.toFixed(0)+'%</td><td>'+fmt(r.pairedSe,1)+'</td><td>'+Number(r.p).toFixed(3)+'</td><td><span class=\"pill '+(r.resolvable?'yes':'no')+'\">'+(r.resolvable?'yes':'no')+'</span></td><td>'+fmt(r.mde,1)+'</td><td>'+r.requiredN+'</td></tr>';}).join('');",
